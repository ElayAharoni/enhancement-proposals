...
- **FR-41:** For failed resources, the detail page must show ResourceStatusLabel in the header (via resource-specific wrapper) and a non-dismissible inline danger Alert below the header with title "Provisioning failed", the API error message as body, and actionLinks for Retry and Delete. The Details tab must show Status and Message in a DescriptionList. Retry re-submits the original creation request (POST) to the same endpoint; the UI does not persist intermediate state—users must re-enter form data if the retry also fails
